...
public class LinkedListDequeTest {
    ...
    public void iteratorTest() {
        ...
        int cnt = 0;
        for(int x : lst) {
            assertEquals("Should have the same value", cnt, x);
            cnt++;
        }
    }
    ...
    public void equalsTest() {
        ...
        assertEquals("Should have the same value", lst1, lst2);

        // 对于两个空的链表，不管内部类型不同，最后仍然是相同的
        LinkedListDeque<Integer> lst3 = new LinkedListDeque<>();
        LinkedListDeque<String> lst4 = new LinkedListDeque<>();
        lst3.addLast(2);
        lst4.addLast("2");
        assertFalse("Should not have the same value", lst3.equals(lst4));
    }
}
